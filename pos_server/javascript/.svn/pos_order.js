...
function safeStock()
{
		...
		$('#product_list').html('<iframe src="/order/safe_stock" style="width:1200px; height:800px"><iframe>')
...
function setProductTable(data,type)
{
	...
	$('#product_order').after('<div id="orderConfirmBtn"  style="height:50px; background-color:#800; text-align:center; width:1240px;"><input type="button" style="float:left" class="big_button" value="確認訂單送出" onclick="orderConfirm()">'+
...
function orderTable(data,type,status)
{
    
	orderKey =false;
	if(type=='print' || type=='boxin') width = '1000';
	else width = '1100';

	content=
	'<div>'+
	'<table border="1" class="orderTableClass" id="orderTable" style="width:'+width+'px;text-align:center" >';
	...
	for( var key in product)
	{
		...
		else if(type=='print') content+=productViewRow(product[key],{count:false,box:false,cabinet:false,orderNum:false,rowDelete:false,shippingStatus:false,eachCost:false});
		else if(type=='noPricePrint') content+=productViewRow(product[key],{count:false,box:false,cabinet:false,orderNum:false,price:false,rowDelete:false,shippingStatus:false,purchaseCount:false,
		purchasePrice:false,subtotal:false,eachCost:false});
		else if(type=='noPriceShowingPrint') content+=productViewRow(product[key],{count:false,box:false,cabinet:false,orderNum:false,rowDelete:false,shippingStatus:false,purchaseCount:false,purchasePrice:false,subtotal:false,eachCost:false});
        else if(type=='pickupList') content+=productViewRow(product[key],{box:false,orderNum:false,price:false,rowDelete:false,shippingStatus:false,purchaseCount:false,purchasePrice:false,subtotal:false,eachCost:false,checkcount:false});
        else if(type=='boxin') content+=productViewRow(product[key],{count:false,orderNum:false,cabinet:true,price:false,rowDelete:false,shippingStatus:false,purchaseCount:false,purchasePrice:false,subtotal:false,eachCost:false,checkcount:false});
         else if(type=='orderChk') content+=productViewRow(product[key],{count:true,orderNum:false,cabinet:true,price:false,rowDelete:false,shippingStatus:false,purchaseCount:false,purchasePrice:false,subtotal:false,eachCost:false,checkcount:false});
...
function orderSend()
{
	...
	if(confirm('請再次確定訂單內容，按確定送出後將無法更改'))
	{
		...
		,receiver:$('#receiver').val(),address:$('#address').val(),phone:$('#phone').val()},function(data)
		{
...
function checkInvoice(id,total,shopID)
{

	$.post('/order/check_invoice',{id:id,total:total,shopID:shopID},
	function(data)
	{
		...
		else
		{
			 ...
			 $('#order_'+id).css('background-color','red');
		}
...
function recommendTable(product)
{
	...
	var Eng = new Array('number','productNum','ZHName','ENGName','language','price','buyDiscount','buyPrice','supplierName','recommendNum','preTime','shopList','subtotal');
		...
		{
			data = product[key];
			content+='<tr id="orderRow_'+data.productID+'">';
			for(row in Zh)
			{
...
function showOrder(orderID,status,type)
{
	...
	//		else content +='<input type="button" class="big_button" value="列印出貨單" onclick="window.open(\'/order/print_out/'+data.order.id+'\', \'_blank\')">';
            ...
            if(type=='staff')
			content +=	'<input id="copyID" value="'+(data.order.orderNum-1)+'"/><input type="button" value="取得資料" onclick="getOrderInformation()">';
;	
				watchType = type;
...
function print_order(orderID)
{
	...
	$.post('/order/get_order',{orderID:orderID},function(data)
	{
		...
		{
			orderNumber = 0;
			var	 content ='<h1 style="text-align:center;">報　價　單</h1><div id="orderAppendQuery"></div>';
			content +='<input type="hidden" id="shopDiscount" value="'+data.order.discount+'">'+
						'<input type="hidden" id="shopID" value="'+data.order.shopID+'">'+	
					'<form id="orderForm">'+
	   ...
	   success: function(data){
		  ...
		  if(data.result==true)
		{
			...
			if(showing==1)
			{
				...
				for(key in data.product )
				{
					...
					if(data.product[key].orderNum!=orderNum)
					{
						...
						if(orderNum!=0)
						{
							if(showtype=='boxin') content+=orderTable(sliceData,'boxin','訂單已送達');
							else if(price==1)content +=orderTable(sliceData,'showingPrint','訂單已送達');
							else content +=orderTable(sliceData,'noPriceShowingPrint','訂單已送達');
			...
			else 
			{
                ...
                if(showtype=='boxin') content+=orderTable(data,'boxin','訂單已送達');
				else if(price==1)content +=orderTable(data,'print','訂單已送達');
				else content +=orderTable(data,'noPricePrint','訂單已送達');
			}
			content +='</form>';
  ...
  $.ajax({
	   ...
	   success: function(data){
		  ...
		  if(data.result==true)
		{
			...
			if(data.order.shopID<1000||data.order.shopID==1045)
			{
				var account = 	'<img src="/images/phalogo.png" style="float:left; height:100px;">'+
                                '<div style="float:left; font-weight:bold;margin-bottom:10px; ">'+
								'幻遊天下股份有限公司<br/>'+
								'瘋桌遊益智遊戲專賣店<br/>'+
								'新北市土城區中央路一段69巷3號<br/>'+
								'電話：02-2263-0120 傳真：02-2263-0110<br/>'+
								'中國信託(822)城東分行<br/>'+
								'幻遊天下股份有限公司 071540245257<br/>'+
            ...
            if(showtype=='boxin') account = '<div style="float:left;font-size:50pt";>'+data.order.shippingNum+'</div><div style="float:left;margin-left:20px; font-weight:bold;margin-bottom:10px; ">請填入裝箱時間：<br/><span style="font-size:60pt">___:___</span><br/></div>';
				 content ='<div id="orderAppendQuery"></div>';
			content +='<input type="hidden" id="shopDiscount" value="'+data.order.discount+'">'+
					 '<form id="orderForm">'+
						...
						'<div style="width:1000px;float:left;">'+
							...
							account+
							'<div style="float:left; margin-left:20px;font-weight:bold; width:350px">'+
							...
							content+='</div>'+
							'<div style="float:left;margin-left:20px;font-size:50pt"><span style="font-size:32pt">'+type+'</span><br/>'+padLeft(data.order.shopID,4)+'</div>'
						'</div>'+
						'<div style="clear:both"></div>'+
						'<div style="text-align:left;">'
			orderNum = 0;
    ...
    $.post('/order/get_select_shipment',{idList:idList},function(data)
	{
		...
		var Today=new Date();
            ...
            for(key in data.orderList)
            {
               ...
               var  r = print_out(data.orderList[key].id,0,data.orderList[key].showPrice,showType);
                
                if(showType!='boxin')r+= '<h2><table border="1" width="1000px"><tr><td>經辦：</td><td>檢核：</td><td>收貨：</td></tr></table></h2>';
      ...
      //  $('.break').css('page-break-after','always');
		 $('.orderTableClass').css('width','1100px');
        $('.orderTableClass').css('font-size','16pt');
...
function productViewRow(data,setting,status)
{
	...
	var Eng = new Array('count','box','checkcount','OSBANum','cabinet','orderNum','ZHName','ENGName','productNum','language','price','purchaseCount','purchasePrice','eachCost','OSBANum','subtotal','orderComment','shippingStatus','rowDelete');
		...
		for(row in Zh)
		{
			...
			if(typeof setting[Eng[row]] == "undefined" ||setting[Eng[row]]==true)
			{
				...
				if(Eng[row]=='count')content+='<td class="countcolumn count_'+data['productID']+'"><input type="button" value=""></td>';
                else if(Eng[row]=='box') content+='<td class="boxcolumn box_'+data['productID']+'"></td>';
				...
				else if(Eng[row]=='orderComment' && status=='採購中')
				{
		...
		content+='</tr>';
        	...
        	if(data.rule==1)content+='<tr style="color:red"><td></td><td>'+data.OSBANum+'</td><td colspan="'+(rownum-2)+'">'+data.ZHName+'   精美手冊</td></tr>';
	...
	var Eng = new Array('number','productNum','ZHName','ENGName','language','price','purchaseCount','purchasePrice','sellNum','comNum','subtotal','orderComment','rowDelete');
		...
		if(orderKey==false)
		{
			content = '<tr style="background-color:#FFEBEB" id="order_header">';
				...
				for( var key in Zh)
				{	
						if(typeof setting[Eng[key]] == "undefined" ||setting[Eng[key]]==true)content+='<td>'+Zh[key]+'</td>';
				}
		...
		parseInt(orderNumber++);
		 ...
		 content+='<tr id="orderRow_'+orderNumber+'" class="">';
		...
		for(row in Zh)
		{
			...
			else if(Eng[row]=='subtotal')
			{
				...
				content+='<td><input type="text"  name="comment'+orderNumber+'" value="" /></td>';
	...
	else
	{
		$.post('/order/get_product_discount',{productID:data.productID,shopID:$('#shopID').val()},function(result)
		{
			if(result.result==true)
			{
			  ...
			  $('#sellNum_'+result.product.productID).val(num);
...
function getOtherMoney()
{
		...
		$.post('/order/get_other_money',{shopID:$('#monthCheck_shopID').val(),year:$('#monthCheck_year').val(),month:$('#monthCheck_month').val()},function(data){
				...
				if(data.result==true)
				{
					$('#check_container').html('<div id="other_msg" style="color:red"></div><table id="otherMoneyTable" border="1"></table><input type="button" onclick="$(\'#new_money\').slideDown()"  class="big_button"  value="新增'+$('#monthCheck_year').val()+'-'+$('#monthCheck_month').val()+'其他款項">');
...
function getHaveMoney()
{
		...
		$.post('/order/get_have_money',{shopID:$('#monthCheck_shopID').val(),year:$('#monthCheck_year').val(),month:$('#monthCheck_month').val()},function(data){
				...
				if(data.result==true)
				{
					$('#check_container').html('<div id="other_msg" style="color:red"></div><table id="otherMoneyTable" border="1"></table><input type="button" onclick="$(\'#new_money\').slideDown()"  class="big_button"  value="新增'+$('#monthCheck_year').val()+'-'+$('#monthCheck_month').val()+'已收款項">');
					$('#check_container').append(
						'<div id="new_money" style=" display:none;background-color:#FEE">'+
					...
					for(key in data.otherMoney)
					{
						...
						'<tr>'+
							...
							'<td>'+data.otherMoney[key].name+'</td>'+
							'<td><input type="text" onchange="updateHaveMoney('+data.otherMoney[key].id+')" id="money_'+data.otherMoney[key].id+'" value="'+data.otherMoney[key].amount+'"></td>'+	
			...
			if(data.result==true)
			{
				...
				for( var key in data.otherShop)
				{
                   ...
                   if(data.otherShop[key].shopID<1000) var shopJoin = '<select onchange="editShopName('+data.otherShop[key].shopID+')" id="shop_join_'+data.otherShop[key].shopID+'">'+
					...
					$('#editShop_invoiceByShip_'+data.otherShop[key].shopID).val(data.otherShop[key].invoiceByShip);
...
function editShopName(shopID)
{
	...
	$.post('/order/edit_shop_name',{shopID:shopID,name:$('#editShopName_'+shopID).val(),distributeType:$('#distributeType_'+shopID).val(),shipOut:$('#shipping_day_'+shopID).val(),address:$('#editShop_address_'+shopID).val(),contactPerson:$('#editShop_contactPerson_'+shopID).val(),phone:$('#editShop_phone_'+shopID).val(),fax:$('#editShop_fax_'+shopID).val(),invoiceType:$('#editShop_invoiceType_'+shopID).val(),invoiceByShip:$('#editShop_invoiceByShip_'+shopID).val(),comID:$('#editShop_comID_'+shopID).val(),email:$('#editShop_email_'+shopID).val(),jointype:$('#shop_join_'+shopID).val()
	...
	$('#product_list').html('');
			$.post('/system/get_shop',{token:1},function(data){
				if(data.result==true)
				{
					...
					//$('#adjust_shopID').append('<option value="0">幻遊天下</option>');
					for( var key in data.shopData)
					{
    ...
    if(!shopID)
    {
		...
		$.post('/order/shipment_view',{shopID:shopID,addressID:addressID},function(data){
			...
			if(data.result==true){
						...
						content+= 	'<div style=" clear:both">收件人：'+data.order['month'][0].receiver+' 地址：'+data.order['month'][0].address+' 電話：'+data.order['month'][0].phone+' <span class="shippingTotal"></span>';
						
						if(typeof(data.order['month'][0]) != 'undefined'&&data.distribute.cashType!=1 )
						{
							content+=   '<input type="button"  class="big_button"  value="匯款後請點此通知" onclick="shipmentOut('+data.order['month'][0].addressID+')">';
						}
						...
						'<label style=" font-size:14pt" ><input type="radio" value="2" id="shipmentStatus5000" name="shipmentStatus"  onclick="shipmentStatusChange()">免運再出貨</label></div>');
								...
								else if(data.distribute.shippingFee>=shippingTotal)
								{
									needShipping = true;
...
function showShipment(shipmentID,type)
{
	...
	$.post('/order/get_shipment',{shipmentID:shipmentID},function(data)
	{
		...
		if(data.result==true)
		{
			...
			content +='<input type="hidden" id="shopDiscount" value="'+data.order.discount+'">'+
					...
					'<form id="shipmentForm">'+
						...
						'<div style="text-align:left">出貨單號：s'+data.order.shippingNum+' 出貨時間：'+data.order.shippingTime+' 訂貨店家：'+data.order.name+' 訂單狀態:'+data.order.status;
			...
			if(type=="staff"&&data.order.status!="已到貨"&&data.order.status!="已送達物流")
			{
   			...
   			content +='訂單狀態：<select name="shipment_status" id="shipment_status">'+
						...
						'</select>';
				content+='物流單號：<input type="text" name="shipmentCode" value="'+data.order.shipmentCode+'">'		
				content+='物流費用：<input type="text" name="shipmentFee" value="'+data.order.shipmentFee+'">'		
			}
			content +='</form>';
		...
		//	{
				...
				content +='/>顯示價錢</label>';
                content +='<input type="button" class="big_button" value="列印裝箱單" onclick="printShipment('+data.order.id+',0,\'boxin\');$(\'#popUpBoxEnter\').trigger(\'click\');">';
            
				var str ='';
			
				content +='<input type="hidden" id="invoiceByShip" value="'+data.order.invoiceByShip+'">';
				content +='<input type="button" class="big_button" value="列印出貨單" onclick="printShipment('+data.order.id+',0,\'shipment\');">';
				content +='<input type="button" class="big_button" value="按訂單列印出貨單" onclick=";printShipment('+data.order.id+',1,\'shipment\');">';
		...
		//	}
			...
			if(type=='staff')
			{
				...
				content +='<input type="button"  id="invoice_print" class="big_button" value="列印發票" onclick="invoicePrepare('+data.order.shopID+','+data.order.total+','+data.order.shippingNum+','+data.order.id+')">';
...
function invoicePrepare(shopID,total,shipmentNum,shipmentID)
{
	...
	$.post('/system/get_shop_status',{shopID:shopID},function(data)
	{
		...
		if(data.result==true)
		{
				...
				'<form id="invoice_form" action="http://localhost/accounting/invoice_api">'+
					...
					'<input type="hidden" id="invoice_item" name="item[]">'+
					'<input type="hidden" id="invoice_uniPrice" name="uniPrice[]">'+	
					'<input type="hidden" id="invoice_num" name="num[]">'+
		...
		if(data.shopData.invoiceType==3)	
		{
...
function printShipment(orderID,type,showType)
{
	...
	if($('#invoiceByShip').val()==1 && showType!='boxin')	
	{
		
		if(confirm('發票將同步印出'))
		{
			$('#invoice_print').trigger('click');
...
{
	if(id==0) return;
	$.post('/order/edit_shipment',{id:id,shipmentFee:$('#shipmentFee_'+id).val(),shipmentCode:$('#shipmentCode_'+id).val(),charge:$('#charge_'+id).val(),note:$('#note_'+id).val()},
	function(data)
	{
...
function getInvoice(id,total,shopID)
{
	$.post('/order/get_invoice',{id:id},function(data)
	{
		if(data.result==true)
		{
			content='<h1>發票清單</h1><form id="invoiceForm">'+
			...
			openPopUpBox(content,600,320,'editInvoice');
			for(i=1;i<=10;i++)
			{
			...
			for( var key in data.shipmentList)
			{
				...
				var content;
					...
					if($('#myshopID').val()==0)
					{
						...
						{
					content+=	
					'<td><input type="text" style=" width:30px" id="shipmentFee_'+data.shipmentList[key].shipmentID+'" value="'+data.shipmentList[key].shipmentFee+'" onfocus="showEditMsg('+data.shipmentList[key].shipmentID+')" onchange="editShipment('+data.shipmentList[key].shipmentID+')"></td>'+
						'<td><input type="number" style=" width:90px"  id="shipmentCode_'+data.shipmentList[key].shipmentID+'" value="'+data.shipmentList[key].shipmentCode+'"  onblur="editShipment('+data.shipmentList[key].shipmentID+')" onfocus="showEditMsg('+data.shipmentList[key].shipmentID+')" ></td>'+
						'<td><input type="text" style=" width:90px"  id="charge_'+data.shipmentList[key].shipmentID+'" value="'+data.shipmentList[key].charge+'"  onfocus="showEditMsg('+data.shipmentList[key].shipmentID+')" ></td>'+
						...
						'<td>'+
							'<input type="button" value="發票" onclick="getInvoice('+data.shipmentList[key].shipmentID+','+data.shipmentList[key].total+','+data.shipmentList[key].shopID+')"class="big_button" >'+
						'</td>'+
							'<td>';
				...
				if(type=='staff')content+='<td><input type="button" id="delBtn_'+data.shipmentList[key].shipmentID+'" value="刪除" onclick="deleteShipment('+data.shipmentList[key].shipmentID+')" class="big_button" ></td>';

						content+='</tr>';
	
				$('#order_list').append(content)
				checkInvoice(data.shipmentList[key].shipmentID,data.shipmentList[key].total,data.shipmentList[key].shopID);
				if($('#myshopID').val()==0)	
				{
...
function shipmentRow(data,type,status,index)
{
	...
	var Zh=new Array('項次','核取','訂單編號','櫃號','商品編號','中文','英文','語言','定價','出貨折數','出貨價格','成本','訂貨數量','出貨數量','公司庫存','小計','備註','預計貨日','刪除');
	var Eng = new Array('number','check','orderNum','cabinet','productNum','ZHName','ENGName','language','price','purchaseCount','purchasePrice','eachCost','buyNum','sellNum','num','subtotal','productComment','shippingDate','rowDelete');
		...
		if(index==1)
		{
		 ...
		 content+='<tr id="orderRow_'+data.rowID+'" style="background-color:'+bgColor+'" >';
		...
		if(data.result==true)
		{
			...
			for( var key in data.product )		   
			{
				...
				content += '<tr id="allocateOrderRow_'+data.product[key].rowID+'"style="background-color:'+bgColor+'">'+
								...
								'<td><input type="hidden" id="shopID_'+key+'" value="'+data.product[key].shopID+'"><input type="hidden" name="orderID_'+key+'" value="'+data.product[key].orderID+'">'+
...
function shipmentOut(addressID)
{
    loginChk();
    
	var sStr = prompt("請輸入匯款金額?");
			if(sStr!=null&&sStr!="")
			{
...
function shoppingCar(placeID)
{
	...
	$('#'+placeID+'Query').html(
		...
		'商品分類<select name="orderType" id="orderType">'+
			...
			'<option value="7">Lc,Fow,魔獸</option>'+
		...
		'<div style=" clear:both"></div>'+
					...
					'<h1>新品入荷<img src="/images/newproduct.jpg"></h1>'+
	...
	$.post('/order/prepayorder',{},function(data){
		...
		if(data.result == true)
		{
			...
			for(key in data.product)
			{
				...
				{
					content+='<a  onMouseover ="showImg(\'http://www.phantasia.tw/phantasia/upload/bg/home/b/'+data.product[key]['phaBid']+'.jpg\')" href="http://www.phantasia.tw/bg/home/'+data.product[key]['phaBid']+' " target="_blank">觀看連結</a>';
...
function shoppingFrame(productData,key,onPage,isMain)
{
	...
	content+='<div class="productBox">';
				...
				content+='<a href="#shoppingProductInfo"><img  style="float:left" onclick ="autoScroll(\'shoppingProductInfo\');getProductInfo('+productData.productID+')" class="productImg" src="'+productData.img+'" onError="this.src=\'http://www.phantasia.tw/phantasia/upload/bg/home/b/0.jpg\';"></a>';
...
function adjustInf(productID,language)
{
	$.post('/order/adjust_amount',{productID:productID},function(data){
			...
			if(data.result==true)
			{
				...
				if(language=='繁中') 
				{
					content+='<h1 style="color:red">此商品為中文商品，請先連絡產品部02-22630120,phantasia.pm@gmail.com<br/>確認到貨時間，再行調貨</h1>';
...
function showProductConsignment()
{
	...
	'<input type="button" value="寄賣商品結算清單"   class="big_button" onclick="consignmentClose()">'+
	'<input type="button" value="寄賣品現在庫存量"   class="big_button" onclick="consignmentNum()">'+
	'<select id="consignmentYear"></select>'+
	'<select id="consignmentMonth"></select>'+
	'<div id="consignmentContnent"></div>'
	
	)
	
	for(i=2014;i>=2011;i--)$('#consignmentYear').append('<option>'+i+'</option>');
	for(i=1;i<=12;i++)$('#consignmentMonth').append('<option>'+i+'</option>');
...
function consignmentAppendSend()
{
	   ...
	   type: "POST",
	   dataType:"json",
	   url: "/order/consignment_product_send",
        ...
        if(data.result==true)
            {
				...
				else
				{
					for( var key in data.collectOrder)
					{
						content='<div class="productBox" style="height:auto">'+nl2br(data.collectOrder[key].comment);
...
function  getRumi()
{
	...
	$.post('/order/get_rumi',{},function(data)
	{
		...
		if(data.result==true)
		{
			...
			if(total>=30000)	str+='<h1>已符合資格</h1>';
			else str+='<h1>還差'+(30000-total)+'方符合辦賽資格</h1>';
			$('#rumiTable').after(str);
...
function getRumiAll(shopID)
{
	...
	$.post('/order/get_rumi',{shopID:shopID},function(data)
	{
		...
		//	$('#rumiTable').append('<tr><td>品名</td><td>1/1後已出貨數量</td><td>金額</td><td>訂購或運送中數量</td><td>金額</td><td>小計</td></tr>');
			...
			if(total>=30000)	str+='<td><h1>已符合資格</h1></td>';
			else str+='<td><h1>還差'+(30000-total)+'方符合辦賽資格</h1></td>';
			$('#rumiTable').append(str);
			if(shopID<40)getRumiAll(shopID+1);
...
function addressUpdate(id,shopID)
{
	...
	$.post('/order/update_address',{id:id,shopID:shopID,receiver:$('#receiver_'+id).val(),address:$('#address_'+id).val(),phone:$('#phone_'+id).val(),defaultToken:isCheck},function(data)
	{
