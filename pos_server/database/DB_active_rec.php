...
class CI_DB_active_record extends CI_DB_driver {
	...
	function delete_backup($sql)
	{
		$Ssql = str_replace('DELETE','SELECT * ',$sql);
		$q = $this->query($Ssql);
		$data = $q->result_array();	
		
		$date = getdate(); 
		
		$dir = $_SERVER['DOCUMENT_ROOT'].'/pos_server/upload/delete/';
		if(!is_dir ($dir)) mkdir($dir);
		
		$file = $dir.'bk_'.date("Y-m-d").'.txt';		
		$output = $sql."\n".json_encode($data)."\n";
		$f = fopen($file,'a');
		fprintf($f,"%s",$output);
		fclose($f);		
